...
function Headbar({ className = "" }) {
  ...
  return (
    <div className="">
      <Grid className={` md:right-[0%] xl:right-[0%] s:relative md:absolute xl:absolute s:top-[-12px] s:right-[15%]  md:top-[24px]  xl:top-[24px]  ${className}`}>
            ...
            <div className="col-span-4 flex self-center justify-around border-r-2 border-[#D1D1D1]">
              ...
              <div>
                <Link to={"/notifications"} className="relative">
                  ...
                  />{" "}
                 {lengthofNotifications == 0 ? (<></>): (<p className="text-[11px] right-[-8px] font-semibold -top-2 rounded-full text-white absolute bg-[red] h-5 w-5 pt-[0px] text-center border-2 border-[#333333]">{lengthofNotifications > 9 ? '9+' : lengthofNotifications}</p>)} 
                </Link>
              </div>
              {/* <div>
